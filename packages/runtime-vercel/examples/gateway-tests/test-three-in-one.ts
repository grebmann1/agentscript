...
async function main(): Promise<void> {
  ...
  const toolCallEvents = turn1.events.filter(e => e.kind === 'tool-call') as Array<
    Extract<RuntimeEvent, { kind: 'tool-call' }>
  >;
  ...
  assertions.eq(errorEvents.length, 0, 'no tool-error events (no hallucinated tools)');
  ...
  const mentionsHours =
    /hour|close|open|today|pm|am|24|don't have|cannot/.test(text);
