...
async function main(): Promise<void> {
  ...
  console.log('--- User: leaks SSN + DOB along with a chest-pain symptom ---\n');
  ...
  console.log(`  final text (truncated)  : ${turn.result.assistantText.slice(0, 200)}`);
  console.log('');
...
function printEvents(events: RuntimeEvent[]): void {
  ...
  for (const e of events) {
    if (e.kind === 'guardrail-fail') {
      attempt++;
      console.log(
        `  [guardrail-fail] attempt ${attempt} reason="${e.error}"`
      );
