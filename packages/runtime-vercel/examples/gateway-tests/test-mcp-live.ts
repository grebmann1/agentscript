 ...
 *   .agent script  →  compileSource (deployment.mcp resolved)
 ...
 *     deployment.mcp.repo = { transport: 'streamable-http', url: env(MCP_URL),
...
import { resolveDeploymentValue } from '@agentscript/compiler';
import type {
...
//    Mirrors `createMcpAdapter()` in `@agentscript/server`; inlined here so
...
function resolveServerSettings(
  ...
  if (!url) throw new Error(`deployment.mcp.${name}.url is required.`);
  const headers: Record<string, string> = {};
  if (server.headers) {
    for (const [k, v] of Object.entries(server.headers)) {
      const r = resolveDeploymentValue(
        v,
        env,
        `deployment.mcp.${name}.headers.${k}`
      );
...
// ---------------------------------------------------------------------------
// 2. Inline agent source — uses deployment.mcp + mcp:// action targets
// ---------------------------------------------------------------------------
...
async function main(): Promise<void> {
  ...
  const deployment = (compiled.output as AgentDSLAuthoringWithDeployment)
    .deployment;
  const mcpServers = deployment?.mcp;
  assertions.truthy(mcpServers, 'compiled deployment.mcp is present');
  assertions.truthy(
    mcpServers && mcpServers.repo,
    'deployment.mcp.repo survives the parser/dialect-schema gate'
  );
  ...
  //    `createMcpAdapter()` in @agentscript/server — inlined so this
