    ...
    agent_version: {
      ...
      nodes: [
        ...
        },
        {
          ...
          tools: [
            { name: 'workA', target: 'workA', description: 'Work A' },
          ],
          action_definitions: [
            {
        ...
        },
        {
          ...
          tools: [
            { name: 'workB', target: 'workB', description: 'Work B' },
          ],
          action_definitions: [
            {
...
describe('Runtime — parallel delegation (delegateMultiple)', () => {
  ...
  it('merges state changes with last-wins strategy (default)', async () => {
    ...
    const llm = new ScriptedLlm([
      { toolCalls: [{ id: 'a1', name: 'setState', arguments: { shared_key: 'from_A' } }] },
      { text: 'A done' },
      { toolCalls: [{ id: 'b1', name: 'setState', arguments: { shared_key: 'from_B' } }] },
      { text: 'B done' },
    ]);
  ...
  it('supports custom merge function', async () => {
    ...
    const llm = new ScriptedLlm([
      { toolCalls: [{ id: 'a1', name: 'setState', arguments: { shared_key: 'A' } }] },
      { text: 'A done' },
      { toolCalls: [{ id: 'b1', name: 'setState', arguments: { shared_key: 'B' } }] },
      { text: 'B done' },
    ]);
  ...
  it('emits delegation-start events for each child', async () => {
    ...
    const childNodes = delegationStarts.map(
      e => (e as any).childNode
    ).sort();
