...
function log(prefix: string, msg: string) {
  console.log(`${DIM}[${new Date().toISOString().slice(11, 23)}]${RESET} ${prefix} ${msg}`);
}
...
function createHttpToolServer(): Promise<{ url: string; server: http.Server }> {
  return new Promise((resolve) => {
    const server = http.createServer(async (req, res) => {
      ...
      log(`${CYAN}[HTTP]${RESET}`, `${req.method} ${path} args=${JSON.stringify(args)}`);
...
function createMcpServer(): Promise<{ url: string; server: http.Server }> {
  ...
  return new Promise((resolve) => {
    const server = http.createServer(async (req, res) => {
      ...
      log(`${YELLOW}[MCP]${RESET}`, `method=${rpc.method} params=${JSON.stringify(rpc.params ?? {})}`);
        ...
        case 'tools/call': {
          ...
          switch (name) {
            case 'calculate': {
              ...
              const num = Function(`"use strict"; return (${expr.replace(/[^0-9+\-*/().]/g, '')})`)();
...
class McpHttpAdapter implements ToolAdapter {
  ...
  async invoke({ target, args }: { target: string; args: Record<string, unknown>; signal?: AbortSignal }): Promise<Record<string, unknown>> {
    await this.initialize();
      ...
      nodes: [
        {
          ...
          tools: [
            { name: 'get_weather', target: `${httpUrl}/weather`, description: 'Get weather' },
            { name: 'web_search', target: `${httpUrl}/search`, description: 'Search the web' },
            { name: 'translate', target: `${httpUrl}/translate`, description: 'Translate text' },
            { name: 'calculate', target: 'mcp://calculate', description: 'Calculate math' },
            { name: 'lookup_db', target: 'mcp://lookup_database', description: 'Look up a DB record' },
            { name: 'notify', target: 'mcp://send_notification', description: 'Send notification' },
          ],
          action_definitions: [],
          before_reasoning: [],
    ...
    agent_version: {
      ...
      nodes: [
        ...
        },
        {
          ...
          tools: [
            { name: 'get_weather', target: `${httpUrl}/weather`, description: 'Get weather' },
            { name: 'calculate', target: 'mcp://calculate', description: 'Calculate' },
          ],
          action_definitions: [],
          before_reasoning: [],
          before_reasoning_iteration: [],
        ...
        },
        {
          ...
          tools: [
            { name: 'web_search', target: `${httpUrl}/search`, description: 'Search' },
            { name: 'lookup_db', target: 'mcp://lookup_database', description: 'Look up DB' },
            { name: 'notify', target: 'mcp://send_notification', description: 'Notify' },
          ],
          action_definitions: [],
          before_reasoning: [],
          before_reasoning_iteration: [],
...
async function runParallelToolCallsTest(httpUrl: string, mcpAdapter: McpHttpAdapter) {
  ...
  const llm = new ScriptedLlm([
    {
      toolCalls: [
        { id: 'c1', name: 'get_weather', arguments: { location: 'Paris', delay: 100 } },
        { id: 'c2', name: 'web_search', arguments: { query: 'AgentScript language', delay: 100 } },
        { id: 'c3', name: 'translate', arguments: { text: 'Hello world', target_lang: 'es', delay: 100 } },
        { id: 'c4', name: 'calculate', arguments: { expression: '42 * 3 + 7' } },
        { id: 'c5', name: 'lookup_db', arguments: { table: 'users', id: 'U-001' } },
        { id: 'c6', name: 'notify', arguments: { user: 'admin', message: 'Test passed!' } },
      ],
    },
  ...
  log(`${GREEN}[PARALLEL]${RESET}`, `Completed in ${BOLD}${parallelTime}ms${RESET}`);
  log(`${GREEN}[PARALLEL]${RESET}`, `Result: "${parallelResult.assistantText}"`);

  const toolResults = events.filter((e) => e.kind === 'tool-result');
  for (const e of toolResults) {
    if (e.kind === 'tool-result') {
      log(`${GREEN}[PARALLEL]${RESET}`, `  Tool ${e.name} → ${JSON.stringify(e.result).slice(0, 80)}...`);
    }
  }
  ...
  console.log(`  ${DIM}(Parallel: ${parallelTime}ms vs Sequential: ${seqTime}ms)${RESET}`);
}

async function runParallelDelegationTest(httpUrl: string, mcpAdapter: McpHttpAdapter) {
  ...
  const llm = new ScriptedLlm([
    ...
    {
      toolCalls: [
        { id: 'w1', name: 'get_weather', arguments: { location: 'Tokyo', delay: 100 } },
      ],
    },
    {
      toolCalls: [
    ...
    {
      toolCalls: [
        { id: 'r1', name: 'web_search', arguments: { query: 'parallel computing', delay: 100 } },
      ],
    },
    {
      toolCalls: [
        { id: 'r2', name: 'lookup_db', arguments: { table: 'articles', id: 'A-123' } },
      ],
    },
    {
      toolCalls: [
        { id: 'r3', name: 'notify', arguments: { user: 'researcher', message: 'Found results' } },
      ],
    },
  ...
  log(`${GREEN}[DELEGATION]${RESET}`, `Completed in ${BOLD}${elapsed}ms${RESET}`);

  for (let i = 0; i < results.length; i++) {
    const r = results[i];
    log(`${GREEN}[DELEGATION]${RESET}`, `  Child ${i}: "${r.assistantText}" (${r.steps} steps, node: ${r.finalNode})`);
    if (Object.keys(r.stateChanges).length > 0) {
      log(`${GREEN}[DELEGATION]${RESET}`, `    State changes: ${JSON.stringify(r.stateChanges)}`);
    }
  }
  ...
  console.log(`  ${BOLD}Both children executed against real HTTP + MCP servers concurrently.${RESET}`);
}
...
async function main() {
  ...
  log(`${DIM}[SETUP]${RESET}`, `MCP tools available: ${mcpTools.map((t) => t.name).join(', ')}`);
  console.log();
